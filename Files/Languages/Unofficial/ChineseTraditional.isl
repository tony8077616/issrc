; *** Inno Setup version 6.0.0+ Chinese (Traditional) messages by Samuel Lee (751555749@qq.com) ,6.0 + Update from MongWu.NeitherHowger.Long***
;
...
; *** Startup questions
PrivilegesRequiredOverrideTitle=選擇安裝程式的安裝模式
PrivilegesRequiredOverrideInstruction=選擇安裝模式
PrivilegesRequiredOverrideText1=%1 可以被所有的使用者安裝 (需要以系統管理員身份執行)，或只給你自己。
PrivilegesRequiredOverrideText2=%1 可以僅被自己安裝，或所有使用者 (需要以系統管理員身份執行)。
PrivilegesRequiredOverrideAllUsers=安裝給所有使用者(&A)
PrivilegesRequiredOverrideAllUsersRecommended=安裝給所有使用者(建議)(&A)
PrivilegesRequiredOverrideCurrentUser=僅安裝給自己(建議)(&M)
PrivilegesRequiredOverrideCurrentUserRecommended=僅安裝給自己(建議)(&M)
...
; *** File copying errors
FileAbortRetryIgnore=點選 [重試] 進行再次重試，[忽略] 來略過這個檔案(不建議)，或 [中止] 來取消安裝。
FileAbortRetryIgnore2=點選 [重試] 進行再次重試，[忽略] 來略過任何程序(不建議)，或 [中止] 來取消安裝。
SourceIsCorrupted=來源檔案已經損毀。
SourceDoesntExist=來源檔案“%1”不存在。
ExistingFileReadOnly2=已存在的檔案無法被取代，因為被標記為唯讀狀態。
ExistingFileReadOnlyRetry=移除唯讀的屬性並再次重試(&R)
